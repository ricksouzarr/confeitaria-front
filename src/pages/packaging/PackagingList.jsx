...
export default function PackagingList() {
    ...
    async function handleDelete(packagingId, packagingName) {
        const confirmed = window.confirm(
            `Tem certeza que deseja excluir a embalagem "${packagingName}"?`
        );

        if (!confirmed) return;

        setDeletingId(packagingId);
        setError("");

        try {
            await api.delete(`/packagings/${packagingId}`);
            setPackagings((prev) => prev.filter((item) => item.id !== packagingId));
        } catch (e) {
            console.error(e);
            setError("Erro ao excluir embalagem.");
        } finally {
            setDeletingId(null);
        }
    }

    const filtered = packagings.filter((item) =>
    ...
    return (
        <MainLayout
            title="Embalagens"
            subtitle="Gerencie as embalagens usadas nas fichas técnicas"
        >
            <div
                style={{
                    display: "flex",
